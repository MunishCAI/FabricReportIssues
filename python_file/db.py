...
class ClientsideDB:
    ...
    def get_moreinfo1(self):
        query = ("select machinedtl_name,machine_type from public.machine_details")    
        data = self.execute.select(query, result_type=2)        
        return data
    ...
    def update_setting1(self, mill,machinetype,machineno,dia,gg,party,needle_drop,count,denier,fabric_type,knittype,gsm_ll,ll,roll_weight,doff,lot,job_no):
        tz = pytz.timezone("Asia/Kolkata")  
        timestamp_ = str(datetime.datetime.now(tz)).replace(" ", "_")
    ...
    def update_setting1(self, mill,machinetype,machineno,dia,gg,party,needle_drop,count,denier,fabric_type,knittype,gsm_ll,ll,roll_weight,doff,lot,job_no):
        ...
        try:
            query1 = f""" 
            SELECT COUNT(*) FROM public.machine_program_details;
            """
            if query1 == 3:
                query2 = f""" 
                SELECT machineprgdtl_id FROM public.machine_program_details ORDER BY timestamp DESC LIMIT 1;
                """ 
                query = f"""
                UPDATE public.machine_program_details SET 
                mill='{mill}', dia='{dia}', gg='{gg}', party='{party}', needle_drop='{needle_drop}', count='{count}', 
                denier='{denier}', fabric_type='{fabric_type}', knit_type='{knittype}', gsm='{gsm_ll}',loop_length='{ll}',
                roll_weight='{roll_weight}', doff='{doff}', lot='{lot}', job_no='{job_no}', timestamp='{timestamp_}',
                machineprogram_sts_id='1' where machineprgdtl_id='{query2}';
                """ 
            else:
                query = f"""
                INSERT INTO machine_program_details (mill,dia,gg,party,needle_drop,count,denier,fabric_type,knit_type,gsm,loop_length,roll_weight,doff,lot,job_no, timestamp, machineprogram_sts_id)
    ...
    def getAlarmSetting(self):
        query = """
                    select dtt.defecttyp_id, dtt.defect_name, adt.minimum_alarm_count, adt.score, adt.alarm_status 
    ...
    def updateAlarmSetting(self,data):
        ...
        for index, row in df.iterrows():
            ...
            # try:
            insert_query = (
            "INSERT INTO public.alarm_definition (alarm_definition_id, defecttyp_id, minimum_alarm_count, score, alarm_status) "
                f"VALUES ('{alarm_definition_id}', '{alarm_definition_id}', '{minimum_alarm_count}','{(int(score)/100)}', '{alarm_status}')"
            )
            if self.execute.insert(insert_query):
    ...
    def updateCameraSetting(self,data):
        ...
        for index, row in df.iterrows():
            ...
            # try:
            insert_query = (
            "INSERT INTO from public.cam_details (cam_id,cam_name, camsts_id) "
