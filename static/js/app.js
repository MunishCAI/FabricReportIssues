...
$(document).ready(function () {
    ...
    // saved program
    program_info_submit_button = document.getElementById("submit")
    saved_program_list_button = document.getElementById("saved_program_no")
    saved_program_list_button.addEventListener('change', function () {
        let saved_program_no = document.getElementById('saved_program_no').value
        console.log('change!!!',saved_program_no)
        $.ajax({
            url: '/update_saved_program_info_api/' + saved_program_no, // Replace with your API endpoint
    ...
    program_info_submit_button.addEventListener('click', function () {
        document.getElementById("program_info_edit_close").click()
        swal({
                title: "Are you want to save it?",
                type: "warning",
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $.ajax({
                        url: '/update_savedprogram',
                        method: 'PUT',
                        success: function (response) {
                            swal("Success!", "Added in saved program!", "success");
                        },
                    ...
                    swal("Success!", "Program info updated!", "success");
                    swal({
                        title: "alert!",
    ...
    let recentRollId = null;
    function setInitialRollId() {
        $.ajax({
            url: '/checkrollcompleted',
            method: 'GET',
            success: function (data) {
                recentRollId = data.rollId;
                console.log('Initial Roll ID set:', recentRollId);
                setInterval(checkRollId, 1000);
            },
            error: function (error) {
                console.error('Error setting initial Roll ID:', error);
            }
        });
    }
    ...
    function checkRollId() {
        $.ajax({
            ...
            method: 'GET',
            success: function (data) {
                const currentRollId = data.rollId;
                console.log('Current Roll ID:', currentRollId);
                console.log('Recent Roll ID:', recentRollId);
    
                if (recentRollId !== currentRollId) {
                
                    swal("Roll is completed");
                    recentRollId = currentRollId; 
                } else {
                    console.log('Condition not met. No SweetAlert.');
                }
            },
            error: function (error) {
                console.error('Error checking roll ID:', error);
            }
        });
    }
