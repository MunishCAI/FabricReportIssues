...
import python_file.alarmLog as alarm_rep
...
@app.route('/update_savedprogram', methods=['PUT']) 
async def update_savedProgram():
        error_message = None
        if request.method == 'PUT':
            print("update_savedprogram")
            db.db.update_saved_program()
            logger.info("INFO : update_savedprogram updated") if logEntry == 1 else None
            print("done update_savedprogram")
            return render_template('index.html')
        
        logger.error("ERROR : update_savedprogram NOT updated") if logEntry == 1 else None
        return render_template('index.html')
...
@app.route('/update_saved_program_info_api/<saved_program_no>', methods=['GET'])
async def update_saved_program_info(saved_program_no):        
        data = db.db.get_saved_info(saved_program_no)
        data1 = db.db.get_moreinfo1()

        try:
            data_dict = {
                "machineprgdtl_id" : data1[2],
                "program_name" : data[1],
                "description" : data[2],
                "gsm_" : data[3], 
                "gg" : data[4],
        ...
        print(data_dict)
        response = jsonify(data_dict)
        logger.info("INFO : update_saved_program_info_api Response") if logEntry == 1 else None
        return response 
...
@app.route('/checkrollcompleted', methods=['GET'])
async def checkrollcompleted():
    try:
        recent_roll_id = db.db.get_rollid()
        return jsonify({'rollId': recent_roll_id})
    except Exception as e:
        print(f"Error fetching recent roll ID: {e}")
        return jsonify({'error': 'Unable to fetch recent roll ID'}), 500
...
def update_config(section, key, value):
    ...
    with open('config_webui.ini', 'w') as configfile:
        config.write(configfile)
        
@app.route('/updatelogoName', methods=['POST'])
def updateLogo():
    if request.method == 'POST':
        try:
            new_logo = request.form['logoName']
            logo_name = f'report_img/{new_logo}'
            update_config('report', 'mill_img', logo_name)
        except Exception as e:
            print('Error in Data Reading:', str(e))
        return render_template('setting.html')
...
@app.route('/alarmlog/<report_date>', methods=['POST', 'GET'])
def alarmlog(report_date):
    if request.method == 'POST':
        datepicker = report_date
        data = alarm_rep.image(datepicker)
        data_dict = {
            "status": data
        }
        response = jsonify(data_dict)
...
@app.route('/alarm_log_viewer_api')
async def alarm_log_viewer():
    directory_path = './static/report'
    for filename in os.listdir(directory_path):
        if filename.endswith(".pdf"):
            current_report = filename
    ...
    destination_file = os.path.join(directory_path, current_report)
    shutil.copy(destination_file, conf_dir)
    return render_template("alarmLogViewer.html",current_report=current_report)
